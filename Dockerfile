...
RUN curl -O `curl -i -s https://wordpress.org/plugins/woocommerce/ | egrep -o "https://downloads.wordpress.org/plugin/[^']+"`
RUN unzip -o woocommerce.*.zip -d /usr/share/nginx/www/wp-content/plugins
RUN chown -R www-data:www-data /usr/share/nginx/www/wp-content/plugins/woocommerce
